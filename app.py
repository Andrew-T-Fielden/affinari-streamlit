...
def user_preferences_view(username):
    ...
    with st.expander("🔢 Scalar Preferences", expanded=True):
        for trait in scalar_traits:
            ...
            current_val = profile.get("scalars", {}).get(trait, 0.5)
            profile.setdefault("scalars", {})[trait] = st.number_input(
                label, 0, 10, int(current_val * 10), step=1
            ) / 10.0

    with st.expander("✅ Tags (Required)", expanded=False):
        for tag in tag_options:
            label = tag.replace('_', ' ').title()
            current_val = profile.get("tags", {}).get(tag, 0)
            profile.setdefault("tags", {})[tag] = 1 if st.checkbox(
                label,
                value=(current_val == 1),
                key=f"tag_{tag}"
            ) else 0
    ...
    with st.expander("🎨 Categoricals", expanded=False):
        ...
        for cat, choices in options.items():
            selected = st.multiselect(
                f"{cat.replace('_', ' ').title()} (optional)",
                choices,
                default=profile.get("categoricals", {}).get(cat, [])
            )
...
def user_scenario_view(base_profile, username):
    ...
    with st.expander("🔢 Scalar Preferences", expanded=True):
        for trait in scalar_traits:
            label = trait.replace('_', ' ').title()
            current_val = base_profile.get("scalars", {}).get(trait, 0.5)
            session_profile.setdefault("scalars", {})[trait] = st.number_input(
                label, 0, 10, int(current_val * 10), step=1, key=f"scalar_{trait}"
            ) / 10.0

    with st.expander("✅ Tags (Required)", expanded=False):
        for tag in tag_options:
            label = tag.replace('_', ' ').title()
            current_val = base_profile.get("tags", {}).get(tag, 0)
            session_profile.setdefault("tags", {})[tag] = 1 if st.checkbox(
                label,
                value=(current_val == 1),
                key=f"tag_{tag}"
            ) else 0
    ...
    with st.expander("🎨 Categoricals", expanded=False):
        ...
        for cat, choices in options.items():
            selected = st.multiselect(
                f"{cat.replace('_', ' ').title()} (optional)",
                choices,
                default=base_profile.get("categoricals", {}).get(cat, []),
                key=f"cat_{cat}"
            )
